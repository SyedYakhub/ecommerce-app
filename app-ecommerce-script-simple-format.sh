...
#you can also add the below lines manually by loggin using -> mariadb -u ecomuser -p -D ecomdb and enter password
#then use cat > db-load-script.sql and add the lines followed by mariadb < db-load-script.sql
...
#then enter this line to view the products -> select * from products;
